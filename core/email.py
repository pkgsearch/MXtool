...
class Email_Bomber:
    ...
    def bomb(self):
        try:
            ...
            print(
                ...
                + "| "
                + Fore.RESET
                + f"Вы выбрали режим бомбы: {self.mode} и {self.amount} электронные письма"
                + Fore.RESET
            )
        except Exception as e:
            print(f"Ошибочка: {e}")
    ...
    def email(self):
        try:
            print(
                Fore.LIGHTCYAN_EX + "| " + Fore.RESET + "Настроечки емаила" + Fore.RESET
            )
            self.server = str(
                input(
                    Fore.LIGHTCYAN_EX
                    + "| "
                    + Fore.RESET
                    + "Выбери сервер почты - 1:Gmail 2:Yahoo 3:Outlook: "
                )
            )
            premade = ["1", "2", "3"]
            default_port = True
